#include "ru_ofh_transmitter.h"
#include "srsran/adt/static_vector.h"
#include "srsran/adt/gps_clock.h"
#include "srsran/instrumentation/traces/ofh_traces.h"

using namespace srsran;
using namespace ofh;

/// Calculates the Open Fronthaul slot symbol point for the given symbol point. An Open Fronthaul slot symbol point is a
/// normal one, but with the SFN values restricted to 1 Byte.
static slot_symbol_point calculate_ofh_slot_symbol_point(slot_symbol_point symbol_point)
{
  // Build a new slot symbol point that manages that the SFN values in OFH is 1 byte.
  slot_point ota_slot = symbol_point.get_slot();
  slot_point ofh_slot(ota_slot.numerology(), ota_slot.sfn() % NOF_OFH_SFNS , ota_slot.slot_index());

  return {ofh_slot, symbol_point.get_symbol_index(), symbol_point.get_nof_symbols()};
}

ofh_transmitter_impl::ofh_transmitter_impl(srslog::basic_logger&                  logger_,
                                                   const ru_window_timing_parameters&     timing_params_,
                                                   std::shared_ptr<ether::gateway>        gw,
                                                   std::shared_ptr<ether::eth_frame_pool> frame_pool) :
  logger(logger_), pool(std::move(frame_pool)), gateway(gw), timing_params(timing_params_)
{
  srsran_assert(gateway, "Invalid Ethernet gateway");
  srsran_assert(pool, "Invalid frame pool");
}

void ofh_transmitter_impl::transmit_frame_burst(span<span<const uint8_t>> frame_burst)
{
  if (frame_burst.empty()) {
    return;
  }

  gateway->send(frame_burst);
  logger.debug("Sending an Ethernet frame burst of size '{}'", frame_burst.size());
}

void ofh_transmitter_impl::enqueue_messages_into_burst(
    const ether::frame_pool_interval&                   interval,
    static_vector<span<const uint8_t>, MAX_BURST_SIZE>& frame_burst)
{
  trace_point pool_access_tp = ofh_tracer.now();
  auto        frame_buffers  = pool->read_frame_buffers(interval);
  ofh_tracer << trace_event("ofh_tx_pool_access", pool_access_tp);

  if (frame_buffers.empty()) {
    return;
  }

  srsran_assert(frame_burst.size() + frame_buffers.size() <= frame_burst.capacity(), "Frame burst vector is too small");

  for (const auto& frame : frame_buffers) {
    frame_burst.emplace_back(frame->data());
  }


  logger.debug("Enqueueing '{}' frame(s) of type '{}-{}' in interval '{}_{}':{}_{} for tx burst",
               frame_buffers.size(),
               (interval.type.type == message_type::uplane_prach) ? "prach" : "user-plane",
               (interval.type.direction == ofh::data_direction::downlink) ? "downlink" : "uplink",
               interval.start.get_slot(),
               interval.start.get_symbol_index(),
               interval.end.get_slot(),
               interval.end.get_symbol_index());
}

void ofh_transmitter_impl::on_new_symbol(slot_symbol_point symbol_point)
{
  trace_point tp = ofh_tracer.now();

  slot_symbol_point ofh_symbol_point = calculate_ofh_slot_symbol_point(symbol_point);

  static_vector<span<const uint8_t>, MAX_BURST_SIZE> frame_burst;

  // Enqueue pending User-Plane messages.
  ether::frame_pool_interval interval_up{{message_type::user_plane, data_direction::uplink},
                                         ofh_symbol_point - timing_params.sym_up_ul_end,
                                         ofh_symbol_point - timing_params.sym_up_ul_start};
  enqueue_messages_into_burst(interval_up, frame_burst);
  // Enqueue pending User-Plane PRACH messages.
  ether::frame_pool_interval interval_up_prach{{message_type::uplane_prach, data_direction::uplink},
                                         ofh_symbol_point - timing_params.sym_up_ul_end,
                                         ofh_symbol_point - timing_params.sym_up_ul_start};
  enqueue_messages_into_burst(interval_up_prach, frame_burst);

  // Transmit the data.
  trace_point tp_ether = ofh_tracer.now();

  transmit_frame_burst(frame_burst);
  ofh_tracer << trace_event("ofh_ether_tx", tp_ether);

  // Clear sent buffers.
  pool->clear_sent_frame_buffers(interval_up);
  pool->clear_sent_frame_buffers(interval_up_prach);

  ofh_tracer << trace_event("ofh_ofh_transmitter", tp);
}
