...
/* Here uplink means downlink*/
 ...
 public:
   uplane_rx_symbol_data_flow_writer(span<const unsigned>                       ul_eaxc_,
                                     ...
                                     std::shared_ptr<downlink_context_repository> dl_context_repo_) :
     ul_eaxc(ul_eaxc_.begin(), ul_eaxc_.end()),
     sector_id(sector_id_),
     logger(logger_),
     dl_context_repo(std::move(dl_context_repo_))
   {
     srsran_assert(!ul_eaxc.empty(), "Invalid number of uplink eAxCs");
     srsran_assert(dl_context_repo, "Invalid uplink context repository");
   }
