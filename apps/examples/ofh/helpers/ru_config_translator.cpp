 ...
 */
 ...
 #include <iostream>
 #include <unistd.h>
 #include "apps/examples/ofh/helpers/worker_manager_config.h"
 #include "ru_config.h"
 #include "ru_config_translator.h"
...
lower_phy_configuration srsran::generate_low_phy_config(const srs_du::du_cell_config& config,
                                                        ...
                                                        unsigned                      max_processing_delay_slot)
 {
   ...
   out_cfg.sector_id                  = 0;
   out_cfg.logger                     = &srslog::fetch_basic_logger("PHY");
  ...
  fmt::print("input_gain_dB = {}\n", out_cfg.amplitude_config.input_gain_dB);
   ...
   frequency_range freq_range = band_helper::get_freq_range(config.dl_carrier.band);
   sector_config.bandwidth_rb = band_helper::get_n_rbs_from_bw(
       MHz_to_bs_channel_bandwidth(config.dl_carrier.carrier_bw_mhz), config.scs_common, freq_range);
 ...
 void srsran::generate_radio_config(radio_configuration::radio&        out_cfg,
                                    const ru_sdr_unit_config&          ru_cfg,
   ...
   for (unsigned sector_id = 0; sector_id != du_cells.size(); ++sector_id) {
     ...
     for (unsigned port_id = 0; port_id != cell.dl_carrier.nof_ant; ++port_id) {
       ...
       if (ru_cfg.device_driver == "zmq") {
         ...
         fmt::format_to(std::back_inserter(buffer), "inproc://{}#{}", getpid(), port_id);
         tx_ch_config.args = to_string(buffer);
       }
     ...
     for (unsigned port_id = 0; port_id != cell.ul_carrier.nof_ant; ++port_id) {
       ...
       if (ru_cfg.device_driver == "zmq") {
         ...
         fmt::memory_buffer buffer;
         fmt::format_to(std::back_inserter(buffer), "inproc://{}#{}", getpid(), port_id);
         rx_ch_config.args = to_string(buffer);
       }
 ...
 void srsran::fill_ru_worker_manager_config(worker_manager_config& config, const ru_sdr_unit_config& ru_cfg)
 {
   ...
   config.config_affinities.resize(sdr_cfg.nof_cells);
   for (unsigned i = 0; i != sdr_cfg.nof_cells; ++i) {
     ...
     config.config_affinities[i].push_back(ru_cfg.expert_execution_cfg.cell_affinities[i].rx_cpu_cfg);
      config.config_affinities[i].push_back(ru_cfg.expert_execution_cfg.cell_affinities[i].timing_cpu_cfg);
   }
   ...
   config.nof_low_prio_threads      = ru_cfg.expert_execution_cfg.threads.non_rt_threads.nof_non_rt_threads;
   config.low_prio_task_queue_size  = ru_cfg.expert_execution_cfg.threads.non_rt_threads.non_rt_task_queue_size;
   config.low_prio_sched_config     = ru_cfg.expert_execution_cfg.affinities.low_priority_cpu_cfg;
 }
